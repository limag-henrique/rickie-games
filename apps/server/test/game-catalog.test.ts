...
it("expõe exatamente os três jogos com instruções", () => {
  ...
  expect(games.map(game=>game.id)).toEqual(["QUEM_SERIA","SE_BEBER","CARTAS_CONTRA_HUMANIDADE"]);
...
it("mantém o manifesto visual completo e as páginas geradas", () => {
  ...
  const publicRoot=fileURLToPath(new URL("../../web/public/content/cartas-contra-humanidade/",import.meta.url));
  for (const page of ["black-01.png","black-05.png","white-01.png","white-26.png"]) expect(existsSync(`${publicRoot}${page}`)).toBe(true);
});
