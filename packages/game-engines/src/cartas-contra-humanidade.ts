...
export type HumanityCommand =
  ...
  | {type:"CHOOSE_WINNER";actorId:string;submissionId:string}
...
export class CartasContraHumanidadeEngine implements GameEngine<HumanityState,HumanityCommand,HumanityPublicView,HumanityPrivateView> {
  ...
  createInitialState(_config:GameConfig,players:Player[]):HumanityState {
    return {phase:"RULES",version:0,players:clone(players),blackCards:clone(this.blackCards),whiteCards:clone(this.whiteCards),blackIndex:0,whiteIndex:0,usedBlackCardIds:[],usedWhiteCardIds:[],rulesAcknowledged:{},hands:{},submissions:{},anonymousSubmissionOrder:[],history:[]};
  }
  ...
  validateCommand(state:HumanityState,command:HumanityCommand):ValidationResult {
    ...
    if (command.type==="CHOOSE_WINNER") return state.phase==="HOST_REVIEW"&&actor.id===state.czarId&&state.anonymousSubmissionOrder.includes(command.submissionId)?{ok:true}:{ok:false,code:"WINNER_FORBIDDEN"};
  ...
  applyCommand(state:HumanityState,command:HumanityCommand):EngineResult<HumanityState> {
    ...
    if (command.type==="ACKNOWLEDGE_RULES") {
      ...
      const winner=next.submissions[Object.keys(next.submissions).find(playerId=>next.submissions[playerId].id===command.submissionId)??""];
      if (!winner) throw new Error("WINNER_FORBIDDEN");
      next.winnerSubmissionId=winner.id; next.winnerPlayerId=winner.playerId;
      next.players=next.players.map(player=>player.id===winner.playerId?{...player,score:player.score+1}:player);
      if (next.currentBlackCard) next.history.push({blackCardId:next.currentBlackCard.id,winnerPlayerId:winner.playerId,submissionId:winner.id,at:this.now()});
      next.phase="ROUND_RESULTS";
  ...
  getPrivateView(state:HumanityState,playerId:string):HumanityPrivateView {
    ...
    const submissionViews=playerId===state.czarId?state.anonymousSubmissionOrder.map(id=>({id,cards:this.cardsForSubmission(state,id)})):undefined;
    return {rulesAcknowledged:Boolean(player&&state.rulesAcknowledged[playerId]),hand,submitted:Boolean(state.submissions[playerId]),submissions:submissionViews,winnerCards:state.phase==="ROUND_RESULTS"&&state.winnerSubmissionId?this.cardsForSubmission(state,state.winnerSubmissionId):undefined};
  }
  ...
  private openRound(state:HumanityState,events:DomainEvent[]):void {
    ...
    state.blackIndex++; state.usedBlackCardIds.push(card.id); state.currentBlackCard=card; state.submissions={}; state.anonymousSubmissionOrder=[]; state.winnerSubmissionId=undefined; state.phase="INPUT_OPEN";
  ...
  private dealToTen(state:HumanityState):void {
    for (const player of this.eligible(state)) {
      const hand=state.hands[player.id]??[]; state.hands[player.id]=hand;
      while (hand.length<10&&state.whiteIndex<state.whiteCards.length) hand.push(state.whiteCards[state.whiteIndex++].id);
    }
  }
