...
export const createRoomSchema = z.object({ gameId:gameIdSchema.default("QUEM_SERIA"), roomName: z.string().trim().min(2).max(40), hostNickname: z.string().trim().min(2).max(18), timerSeconds:z.number().int().min(5).max(180).optional() });
...
export const commandSchema = z.discriminatedUnion("type", [
  ...
  z.object({ type:z.literal("REVEAL_TURN_CARD"), ...commandBase }),
  ...
  z.object({ type:z.literal("CHOOSE_WINNER"), ...commandBase, submissionId:z.string().min(1) }),
